...
export async function streamingResearchReport(
  ...
  try {
    ...
    const inputData = {
      basicInfo: data.basicInfo,
      financialData: {
        income: data.financialData.income.slice(0, 3),
        balance: data.financialData.balance.slice(0, 3),
        cashflow: data.financialData.cashflow.slice(0, 3),
        indicators: data.financialData.indicators.slice(0, 3)
      },
      marketData: {
        latest: data.marketData[data.marketData.length - 1],
        oldest: data.marketData[0],
